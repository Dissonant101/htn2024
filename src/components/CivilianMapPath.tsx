...
export default function CivilianMapPath() {
  ...
  const getNearestFireExitPath = () => {
    if (!currentPosition || !mapView) return null;
  
    let minDistance = Number.MAX_VALUE;
    let minPath = null;
    for (let i = 0; i < fireExits.length; i++) {
      const currentPath = mapView.getDirections(mapView.createCoordinate(currentPosition.position.latitude!, currentPosition.position.longitude!), fireExits[i]);
  ...
  useEffect(() => {
    if (mapData) {
      ...
      const filteredFireExits = pointsOfInterest.filter(p => p.name.includes("Fire Exit"));
